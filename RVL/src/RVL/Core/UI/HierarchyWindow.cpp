...
namespace Rvl
{
    ...
    HierarchyWindow::HierarchyWindow(const Ref<Scene>& scene, const std::string& name)
        : _scene(scene), _name(name)
    {
        _selected = _scene->GetEntities()[0];
    }
    ...
    void HierarchyWindow::SetName(const std::string& name)
    {
        _name = name;
    }

    std::string HierarchyWindow::GetName() const
    {
        return _name;
    }
}
